...
if [ ! -f /opt/boot/OrangePiH5_ZeroPlus2_EMMC_boot0.bin ]; then
    echo "Error: /boot/boot0.bin not found."
    exit 1
fi
if [ ! -f /opt/boot/OrangePiH5_ZeroPlus2_EMMC_uboot.bin ]; then
...
dd if=/opt/boot/OrangePiH5_ZeroPlus2_EMMC_boot0.bin of=${sdcard} bs=1k seek=8 > /dev/null 2>&1
if [ $? -ne 0 ]; then
    echo "ERROR installing u-boot."
    exit 1
fi
dd if=/opt/boot/OrangePiH5_ZeroPlus2_EMMC_uboot.bin of=${sdcard} bs=1k seek=16400 > /dev/null 2>&1
if [ $? -ne 0 ]; then
echo "ERROR installing u-boot."
exit 0
fi
...
if [ ! -d /media/root/BOOT/ ]; then
    mkdir -p /media/root/BOOT/
fi
mount ${Tcard}p1 /media/root/BOOT/
#-----------------------------------------------------------------------------------------
